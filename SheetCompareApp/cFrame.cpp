...
cFrame::cFrame() : wxFrame(nullptr, wxID_ANY, "Spreadsheet Compare - Erasca", wxPoint(100, 100), wxSize(340, 170), wxDEFAULT_FRAME_STYLE & ~(wxRESIZE_BORDER | wxMAXIMIZE_BOX))
{
	...
	pastText = new wxStaticText(this, wxID_ANY, "Old Spreadsheet: ", wxPoint(10, 2));
	pastFile = new wxFilePickerCtrl(this, 10002, "", "", "XLSX and XLS files (*.xlsx;*.xls)|*.xlsx;*.xls", wxPoint(10, 20), wxSize(300, 20));
	recentText = new wxStaticText(this, wxID_ANY, "New Spreadsheet: ", wxPoint(10, 42));
	recentFile = new wxFilePickerCtrl(this, 10003, "", "", "XLSX and XLS files (*.xlsx;*.xls)|*.xlsx;*.xls", wxPoint(10, 60), wxSize(300, 20));
...
void cFrame::PerformTransfer(wxCommandEvent& evt)
{
	...
	std::wstring pastPath = pastFile->GetPath().ToStdWstring();
	std::wstring recentPath = recentFile->GetPath().ToStdWstring();
	auto rowText = rowInput->GetLineText(0).ToStdString();
	if (pastPath.find(L".xls") == std::wstring::npos)
	...
	if (!rowText.empty())
	{
		...
		Compare* comp = new Compare(pastPath, recentPath, row);
		if (comp->isID())
		{
			comp->CompareBooks();
			delete comp;
